...
class Player(Entity):
	...
	destroy_attack,create_magic):
		...
		# stats
		self.base = {'health': 100,'energy':60,'attack':0,'magic':2,'speed':1,
		'strength':2,'agility':2,'stamina':200,
		'hp_regen':0.1,'crit_chance':10,'crit_rate':120}
		self.current_stats = {}
		for x in self.base.keys():
			self.current_stats[x] = self.base[x]
		
		self.max_stats = {'health': 300, 'energy': 140, 'attack': 20, 'magic' : 10, 'speed': 10,'strength': 10,'agility':10,'stamina':500,'hp_regen':0.5,'crit_chance':30,'crit_rate':250}
		self.upgrade_cost = {'health': 100, 'energy': 100, 'attack': 100, 'magic' : 100, 'speed': 100,'strength':100,'agility':100,'stamina':100,'hp_regen':100,'crit_chance':100,'crit_rate':100}
	...
	def get_full_weapon_damage(self):
		...
		self.current_stats['attack'] = weapon_damage*self.current_stats['strength']
	
		if chance(self.current_stats['crit_chance']):
			modify = convert_to_num(self.current_stats['crit_rate'])
			print('crit',self.current_stats['attack']*modify)
		else:
			modify = 1
		return self.current_stats['attack']*modify
	...
	def get_value_by_index(self,index):
		return list(self.base.values())[index]
	...
	def update(self):
		...
		self.get_status()
		self.animate()
		self.move(self.current_stats['speed']*self.current_stats['agility'])
