...
def test_biome_types():
    """Тест типов биомов"""
        ...
        try:
        from systems.w or ld.biome_types import BiomeType, ClimateType
        WeatherType
...
def test_location_manager():
    """Тест менеджера локаций"""
        ...
        if result:
        prin t("✅ Локация успешно добавлена")
        ...
        if dis cover_result:
        prin t("✅ Локация успешно открыта")
        ...
        if location:
        prin t(f"   Локация получена: {location.name}")
...
def ma in():
    """Основная функция тестирования"""
        ...
        for test_name, test_funcin tests:
        try:
        if test_func():
        passed = 1
        prin t(f"✅ Тест '{test_name}' прошел успешно")
        else:
        prin t(f"⚠️  Тест '{test_name}' не прошел")
        except Exception as e:
        pass
        pass
        pass
