...
class GameMenu:
    ...
    def _init_submenus(self):
        ...
        self.submenus = {
            'inventory': {
                'weapons': MenuItem("Оружие", lambda: None, description="Боевое снаряжение"),
                'armor': MenuItem("Броня", lambda: None, description="Защитное снаряжение"),
                'consumables': MenuItem("Расходники", lambda: None, description="Зелья и свитки"),
                'materials': MenuItem("Материалы", lambda: None, description="Крафтовые ресурсы")
            },
            'character': {
                'attributes': MenuItem("Характеристики", lambda: None, description="Основные параметры"),
                'stats': MenuItem("Статистика", lambda: None, description="Игровая статистика"),
                'equipment': MenuItem("Экипировка", lambda: None, description="Надетые предметы"),
                'leveling': MenuItem("Прокачка", lambda: None, description="Развитие персонажа")
            },
            'skills': {
                'combat': MenuItem("Боевые", lambda: None, description="Атакующие способности"),
                'passive': MenuItem("Пассивные", lambda: None, description="Постоянные эффекты"),
                'special': MenuItem("Особые", lambda: None, description="Уникальные умения")
            },
            'settings': {
                'graphics': MenuItem("Графика", lambda: None, description="Настройки отображения"),
                ...
                'gameplay': MenuItem("Игровой процесс", lambda: None, description="Игровые настройки")
            }
        }
    ...
    def _draw_main_menu(self):
        ...
        self.parent_canvas.create_rectangle(
            menu_x, menu_y, 
            menu_x + layout['menu_width'], menu_y + layout['menu_height'],
            fill=colors['bg'],
        ...
        self.parent_canvas.create_text(
            menu_x + layout['menu_width'] // 2, menu_y + 30,
            text="ИГРОВОЕ МЕНЮ",
            font=fonts['title'],
            fill=colors['text'],
            tags="menu"
        ...
        self.parent_canvas.create_line(
            menu_x + layout['padding'], menu_y + 60,
            menu_x + layout['menu_width'] - layout['padding'], menu_y + 60,
            fill=colors['border'], width=2, tags="menu"
        )
    ...
    def _draw_submenu(self):
        ...
        self.parent_canvas.create_rectangle(
            ...
            back_btn_x + back_btn_width, back_btn_y + back_btn_height,
        ...
        self.parent_canvas.create_text(
            back_btn_x + back_btn_width // 2, back_btn_y + back_btn_height // 2,
            text="← Назад",
            font=fonts['submenu'],
            fill=colors['text'],
            tags="menu"
    ...
    def _draw_menu_items(self, x: int, y: int, items: Dict[str, MenuItem], 
                        item_height: int, spacing: int):
        ...
        for i, (key, item) in enumerate(items_list):
            ...
            # Фон элемента
            self.parent_canvas.create_rectangle(
                x + layout['padding'], item_y,
                x + layout['menu_width'] - layout['padding'], item_y + item_height,
                fill=bg_color,
                outline=colors['border'],
            ...
            self.parent_canvas.create_text(
                x + layout['padding'] + 10, item_y + item_height // 2,
                text=item.label,
                font=fonts['menu'],
                fill=text_color,
                anchor='w',
            ...
            if item.description:
                self.parent_canvas.create_text(
                    x + layout['padding'] + 10, item_y + item_height // 2 + 15,
            ...
            if item.shortcut:
                self.parent_canvas.create_text(
                    x + layout['menu_width'] - layout['padding'] - 10, item_y + item_height // 2,
                    text=item.shortcut,
                    font=fonts['submenu'],
                    fill=colors['accent'],
                    anchor='e',
    ...
    def _on_click(self, event):
        ...
        if self.state == MenuState.SUBMENU:
            ...
            back_btn_x = menu_x + layout['padding']
            back_btn_y = menu_y + layout['padding']
            if (back_btn_x <= x <= back_btn_x + 80 and 
                back_btn_y <= y <= back_btn_y + 30):
                self.current_submenu = None
                self.state = MenuState.MAIN
                self.selected_index = 0
                self._draw_menu()
    ...
    def _handle_item_click(self, x: int, y: int, menu_x: int, menu_y: int, 
                          items: Dict[str, MenuItem], start_y: int):
        """Обработка клика по элементам меню"""
        layout = self.styles['layout']
        items_list = list(items.items())
        
        for i, (key, item) in enumerate(items_list):
            item_y = menu_y + start_y + i * (layout['item_height'] + layout['item_spacing'])
            
            if (menu_x + layout['padding'] <= x <= menu_x + layout['menu_width'] - layout['padding'] and 
                item_y <= y <= item_y + layout['item_height']):
