...
class ExampleEntity:
    ...
    def __init__(self, name: str, entity_type: str, position: tuple):
        ...
        self.combat_stats = {"damage": 10, "defense": 5, "speed": 100}
        ...
        self.skills = {
            ...
            "heal": {"id": "heal", "healing": 20, "cooldown": 5.0, "tags": ["heal"]},
        }
    ...
    def take_damage(self, damage: float):
        ...
        self.ai_core.record_action(
            "take_damage", True, {"damage": damage, "health_remaining": self.health}
        )
    ...
    def heal(self, amount: float):
        ...
        # Записываем в AI память
        self.ai_core.record_action(
            "heal", True, {"healing": amount, "health_gained": self.health - old_health}
        )
    ...
    def use_skill(self, skill_id: str):
        ...
        self.ai_core.record_action(
            "use_skill",
            True,
            {"skill_id": skill_id, "skill_type": skill.get("tags", [])},
        )
    ...
    for i in range(10):
        ...
        for entity in [warrior, archer, mage]:
            if entity.alive:
                ...
                logger.info(
                    f"{entity.name}: {ai_summary['state']}, "
...
def example_adaptive_behavior():
    ...
    scenarios = [
        (
            "Успешные атаки",
            lambda: adaptive_entity.ai_core.record_action("attack", True, {}),
        ),
        (
            "Неудачные атаки",
            lambda: adaptive_entity.ai_core.record_action("attack", False, {}),
        ),
        ("Получение урона", lambda: adaptive_entity.take_damage(25)),
        ("Лечение", lambda: adaptive_entity.heal(30)),
        (
            "Успешная защита",
            lambda: adaptive_entity.ai_core.record_action("defend", True, {}),
        ),
    ]
    ...
    for scenario_name, action in scenarios:
        ...
        logger.info(
            f"Агрессия: {current_personality.aggression:.2f} "
            f"(изменение: {current_personality.aggression - initial_personality.aggression:+.2f})"
        )
        logger.info(
            f"Осторожность: {current_personality.caution:.2f} "
