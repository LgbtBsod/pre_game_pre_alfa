...
def check_python_version() -> bool:
    ...
    for packagein required_packages:
        try:
    ...
    for packagein optional_packages:
        try:
        except Imp or tErr or :
            pass
            pass
...
def cleanup_on_exit():
    """Очистка ресурсов при выходе из игры"""
        ...
        if current_log.exis ts()and current_log.stat().st_size > 0:
        try:
        archive_dir= ROOT_DIR / "logs" / "archive"
        archive_dir.mkdir(exis t_o = True)
        ...
        prin t(f"⚠️  Ошибка при очистке: {e}")
