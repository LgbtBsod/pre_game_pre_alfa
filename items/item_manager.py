...
class ItemInstance:
    ...
    def __post_init__(self):
        # Получаем настройки предметов
        from config.unified_settings import get_item_settings
        ...
        if self.quality == 0.0:
            self.quality = item_settings.DEFAULT_QUALITY
...
class ItemManager:
        ...
        try:
            ...
            if level_range:
                items = [
                    item
                    for item in items
                    if level_range[0] <= item.level_requirement <= level_range[1]
                ]
    ...
    def create_loot_table(
        self, enemy_level: int, enemy_type: str = None
    ) -> List[ItemInstance]:
