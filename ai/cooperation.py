...
class AICoordinator:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.strategy_weights = {
                "FLANKING_MANEUVER": 1.0,
                "DEFENSIVE_FORMATION": 1.0,
                "STANDARD_ATTACK": 1.0,
                "AM_BUSH": 1.0
            }
    ...
    def update_group_behavior(self, group_id):
        ...
        available_strategies = ["FLANKING_MANEUVER", "DEFENSIVE_FORMATION", "STANDARD_ATTACK"]
    ...
    def log_combat_result(self, group_id, strategy, success):
        ...
        if success:
            self.strategy_weights[strategy] = min(3.0, self.strategy_weights[strategy] * 1.2)
        else:
            self.strategy_weights[strategy] = max(0.2, self.strategy_weights[strategy] * 0.8)
...
class GroupTactics:
    ...
    FORMATION_POSITIONS = {
        ...
        "AM_BUSH": {
            ...
            "hidden": lambda leader: (leader.position[0] + random.uniform(-3, 3), leader.position[1] + random.uniform(-3, 3))
        }
    }
    ...
    def execute_action(cls, action, entity):
        if action == "coordinate_flank":
            ...
            support_units = [e for e in AICoordinator().groups[entity.group_id] if e.role == "SUPPORT"]
            if support_units:
...
class SiegeTactics:
    ...
    def execute_gate_breach(breacher, archers, gate):
        """Прорыв ворот"""
        if breacher.has_ability("BREACH") and all(a.has_ability("COVER_FIRE") for a in archers):
...
class TacticalAnalyzer:
    ...
    def adapt_to_player_style(self, player_actions):
        ...
        self.player_tendencies["flanking"] = sum(1 for a in player_actions if "FLANK" in a) / total_actions
        self.player_tendencies["ranged"] = sum(1 for a in player_actions if "RANGED" in a) / total_actions
    ...
    def activate_formation(self, group_id, formation_name, leader):
        ...
        if formation_name in self.formations:
            ...
            for role, offset in self.formations[formation_name].items():
                positions[role] = (leader.position[0] + offset[0], leader.position[1] + offset[1])
