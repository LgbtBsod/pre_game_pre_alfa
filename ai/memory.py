...
class LearningController:
    ...
    def _get_state_hash(self):
        ...
        if hasattr(self.entity, "nearby_enemies"):
            ...
            try:
                enemy_near = (
                    len(self.entity.get_nearby_entities(radius=15.0, enemy_only=True))
                    > 0
                )
            except Exception:
                enemy_near = False
    ...
    def _calculate_reward(self):
        ...
        if (
            hasattr(self.entity, "last_action_success")
            and self.entity.last_action_success
        ):
            reward += 1.0

        # Штраф за потерю здоровья
        if (
            hasattr(self.entity, "last_health")
            and self.entity.health < self.entity.last_health
        ):
        ...
        if hasattr(self.entity, "last_mana_used"):
            efficiency = self.entity.last_mana_used / max(
                1, self.entity.last_damage_dealt
            )
            if efficiency < 0.5:
                reward += 0.3
    ...
    def _perform_action(self, action):
        if action == "ATTACK":
            ...
            if hasattr(self.entity, "combat_stats"):
                self.entity.combat_stats["defense"] = min(
                    20, self.entity.combat_stats.get("defense", 5) + 1
                )
            ...
            if hasattr(self.entity, "position") and hasattr(
...
class EnhancedAIMemory(AIMemory):
    def __init__(self, short_term_capacity=10, long_term_capacity=100):
        ...
        # Используем значение по умолчанию вместо неопределенного UnifiedSettings
        self.memory_decay_rate = 0.01  # MEMORY_DECAY_RATE
