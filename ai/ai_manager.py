...
class AIManager:
    ...
    def __init__(self):
        ...
        self.max_active_entities = 1000  # Максимальное количество активных сущностей
        self.max_update_time_per_frame = 0.016  # 16ms per frame
        self.update_budget = 0.1  # 100ms total budget
    ...
    def _select_entities_for_update(self) -> List[int]:
        ...
        sorted_entities = sorted(
            self.entities.items(),
            key=lambda x: (x[1].priority.value, current_time - x[1].last_update),
        )
    ...
    def get_nearby_entities(
        self, position, radius: float, entity_type: str = None
    ) -> List:
                ...
                if entity_id in self.entities:
                    ...
                    if entity_type:
                        if (
                            ...
                            continue
                        if (
                            hasattr(entity, "enemy_type")
                            and entity.enemy_type != entity_type
                        ):
                            continue
    ...
    def get_performance_stats(self) -> Dict:
        ...
        with self._lock:
            ...
            stats["active_entities"] = len(
                [e for e in self.entities.values() if e.is_active]
            )
    ...
    def _update_performance_stats(self):
        ...
        if current_time - self.performance_stats["last_stats_time"] > 5.0:
            ...
            logger.info(
                f"AI Stats: {stats['active_entities']}/{stats['total_entities']} "
...
class SpatialHash:
    ...
    def __init__(self, cell_size: float = 100.0):
        ...
        self.cell_size = cell_size if cell_size > 0 else 100.0
    ...
    def add_entity(self, entity_id: int, position):
        ...
        self.cells[cell_key].add(entity_id)
        ...
        for dx in range(-cells_to_check, cells_to_check + 1):
            for dy in range(-cells_to_check, cells_to_check + 1):
                ...
                if cell_key in self.cells:
                    entities.update(self.cells[cell_key])
