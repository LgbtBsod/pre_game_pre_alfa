...
class AdvancedAIController:
    ...
    ENTITY_PRIORITY = {"PLAYER": 0, "BOSS": 1, "MINIBOSS": 2, "ELITE": 3, "NORMAL": 4}
    ...
    def __init__(self, entity):
        ...
        self.decision_priority = ["SURVIVAL", "GROUP_ORDER", "PERSONAL_GOAL"]
        ...
        self.emotion_genetics = EmotionGeneticSynthesizer(
            self.entity, self.genetic_profiles, self.abilities, self.effects
        )
    ...
    def _determine_update_mode(self):
        ...
        if (
            hasattr(self.entity, "distance_to_player")
            and self.entity.distance_to_player < 1000
        ):
    ...
    def _full_update(self, delta_time: float, current_time: float):
        ...
        self.memory.record_event(
            "BEHAVIOR_EXECUTION",
            {"status": behavior_status.name, "strategy": self.current_strategy},
        )
    ...
    def _adapt_based_on_experience(self):
        ...
        if self.memory.count_failures("ATTACK") > 3:
            self.personality["aggression"] = max(
                0.1, self.personality["aggression"] - 0.15
            )
            self.memory.record_event(
                "PERSONALITY_CHANGE",
                {"trait": "aggression", "change": -0.15, "reason": "repeated_failures"},
            )

        # Увеличение агрессии после успехов
        if self.memory.count_successes("ATTACK") > 5:
            self.personality["aggression"] = min(
                0.9, self.personality["aggression"] + 0.1
            )
            self.memory.record_event(
                "PERSONALITY_CHANGE",
                {"trait": "aggression", "change": +0.1, "reason": "successful_attacks"},
            )
        ...
        if self.entity.health < 0.4:
            ...
            self.memory.record_event(
                "PERSONALITY_CHANGE",
                {"trait": "caution", "change": +0.2, "reason": "low_health"},
            )
    ...
    def _healing_behavior(self):
        ...
        elif hasattr(self.entity, "has_ability") and self.entity.has_ability(
            "SELF_HEAL"
        ):
    ...
    def _random_movement(self):
        ...
        self.entity.move_towards(
            target_pos, speed, 0.1
        )  # delta_time = 0.1 для плавности
    ...
    def _select_and_use_ability(self):
        ...
        if self.entity.health < self.entity.max_health * 0.4:
            healing_skills = self._find_skills_by_tags(
                ["heal", "restore", "regeneration"]
            )
            if healing_skills:
        ...
        if self.threat_level > 15:
            defense_skills = self._find_skills_by_tags(
                ["defense", "shield", "barrier", "evasion"]
            )
            if defense_skills:
    ...
    def _is_skill_available(self, skill_id: str, skill_data: dict) -> bool:
        ...
        if (
            "requires_target" in skill_data.get("conditions", [])
            and not self.entity.target
        ):
            return False
