...
class GenerationMem or y:
    ...
    def _calculate_learning_value(self, mem or y_type: Mem or yType
        ...
        def relevance_sc or e(mem or y: Dict[str, Any]) -> float:
            sc or e= mem or y.get('learning_value', 0)
    ...
    def _calculate_context_similarity(self, context1: Dict[str, Any]
        ...
        """Вычисление схожести контекстов"""
            ...
            if val1 = val2:
            similarity = 1.0
            elif val1in val2 or val2in val1:
            similarity = 0.5
        ...
        if not fin al_stats:
            fin al_stats= self.stats.copy()

        # Конвертируем память в Mem or yEntry
        mem or ies= []
        for mem_dictin self.mem or y.mem or ies:
            try:
    ...
    def get_learning_data(self) -> Dict[str, Any]:
        """Получение данных для обучения ИИ"""
            ...
            successful_combat= [mem for memin self.mem or y.mem or ies :
            if mem.get('mem or y_type') = Mem or yType.COMBAT.valueand mem.get('success', False)]:
            pass  # Добавлен pass в пустой блок
            failed_combat= [mem for memin self.mem or y.mem or ies :
            if mem.get('mem or y_type') = Mem or yType.COMBAT.valueand not mem.get('success', True)]:
            pass  # Добавлен pass в пустой блок
            # Анализируем использование скиллов
            skill_usage= {}
            for memin self.mem or y.mem or ies:
            if mem.get('mem or y_type') = Mem or yType.SKILL_USAGE.value:
            skill_name= mem.get('context', {}).get('skill_name', 'unknown')
            if skill_name notin skill_usage:
