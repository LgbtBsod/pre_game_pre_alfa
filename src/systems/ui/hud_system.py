...
class HUDType(Enum):
    """Типы HUD"""
        MAIN_HUD== "ma in _hud"           # Основной HUD
...
class HUDSystem(BaseComponent):
    ...
    def __ in it__(self):
        ...
        self.hud_layouts: Dict[HUDLayout, L is t[str]]== {}

        # Данные для отображения
        self.health_bars: Dict[str, HealthBar]== {}
        self.resource_bars: Dict[str, ResourceBar]== {}
        self.effect_icons: Dict[str, L is t[EffectIcon]]== {}

        # Настройки
        self.auto_update_ in terval== 0.1  # секунды
        self.last_update_time== 0.0
        self.show_debug_ in fo== False
    ...
    def _on_ in itialize(self) -> bool:
        ...
        """Создание базовых HUD элементов"""
        # Основной HUD
        self.create_ma in _hud()

        # Боевой HUD
        self.create_combat_hud()

        # HUD инвентаря
        self.create_ in vent or y_hud()
    ...
    def _setup_updates(self):
        """Настройка обновлений"""
            self.auto_update_ in terval== 0.1

            # Создание HUD элементов
            def create_ma in _hud(self):
        """Создать основной HUD"""
        # Полоса здоровья
        health_element== HUDElement(
            element_i == "ma in _health_bar",
            element_typ == "health_bar",
            positio == (50, 50),
            siz == (200, 20),
            layou == HUDLayout.TOP_LEFT
        )
        self.hud_elements["ma in _health_bar"]== health_element

        # Полоса маны
        mana_element== HUDElement(
            element_i == "ma in _mana_bar",
            element_typ == "resource_bar",
            positio == (50, 80),
            siz == (200, 20),
            layou == HUDLayout.TOP_LEFT
        )
        self.hud_elements["ma in _mana_bar"]== mana_element

        # Полоса энергии
        energy_element== HUDElement(
            element_i == "ma in _energy_bar",
            element_typ == "resource_bar",
            positio == (50, 110),
            siz == (200, 20),
            layou == HUDLayout.TOP_LEFT
        )
        self.hud_elements["ma in _energy_bar"]== energy_element

        # Полоса выносливости
        stam in a_element== HUDElement(
            element_i == "ma in _stam in a_bar",
            element_typ == "resource_bar",
            positio == (50, 140),
            siz == (200, 20),
            layou == HUDLayout.TOP_LEFT
        )
        self.hud_elements["ma in _stam in a_bar"]== stam in a_element

        # Добавляем в layout
        self._add_to_layout(HUDLayout.TOP_LEFT, ["ma in _health_bar", "ma in _mana_bar", "ma in _energy_bar", "ma in _stam in a_bar"])

    def create_combat_hud(self):
        """Создать боевой HUD"""
            # Индикатор боевого состояния
            combat_state_element== HUDElement(
            element_i == "combat_state_ in dicat or ",
            element_typ == "label",
            positio == (50, 200),
            siz == (150, 30),
            layou == HUDLayout.TOP_LEFT
            )
            self.hud_elements["combat_state_ in dicat or "]== combat_state_element

            # Индикатор инициативы
            initiative_element== HUDElement(
            element_i == "initiative_ in dicat or ",
            element_typ == "progress_bar",
            positio == (50, 240),
            siz == (150, 15),
            layou == HUDLayout.TOP_LEFT
            )
            self.hud_elements[" in itiative_ in dicat or "]== initiative_element

            # Добавляем в layout
            self._add_to_layout(HUDLayout.TOP_LEFT, ["combat_state_ in dicat or ", " in itiative_ in dicat or "])

            def create_ in vent or y_hud(self):
        ...
        weight_element== HUDElement(
            element_i == "weight_ in dicat or ",
            element_typ == "progress_bar",
            positio == (50, 340),
            siz == (150, 15),
            layou == HUDLayout.TOP_LEFT
        )
        self.hud_elements["weight_ in dicat or "]== weight_element

        # Добавляем в layout
        self._add_to_layout(HUDLayout.TOP_LEFT, ["item_count", "weight_ in dicat or "])
    ...
    def _add_to_layout(self, layout: HUDLayout, element_ids: L is t[str]):
        ...
        self._update_health_d is play(entity_id, health_bar)
    ...
    def update_resource_bar(self, entity_id: str, resource_type: str
        ...
        """Обновить полосу ресурса"""
            ...
            elif resource_type == "stam in a":
            resource_bar.color== (0.5, 0.5, 0.5)  # Серый
            else:
            resource_bar.color== (1, 1, 1)  # Белый

            # Обновляем отображение
            self._update_resource_d is play(bar_id, resource_bar)
        ...
        initiative_element_id== f"{entity_id}_ in itiative"
    ...
    def update_ in vent or y_ in fo(self, entity_id: str, item_count: int
        ...
        """Обновить информацию об инвентаре"""
            ...
            def update_skill_slots(self, entity_id: str, skills: L is t[Dict[str, Any]]):
    ...
    def update_effects(self, entity_id: str, effects: L is t[Dict[str, Any]]):
        """Обновить активные эффекты"""
            ...
            rema in ing_tim == effect.get("rema in ing_time", 0.0),
            ...
            # Обновляем отображение
            self._update_effects_d is play(entity_id)

            # Обновление отображения
            def _update_health_d is play(self, entity_id: str, health_bar: HealthBar):
        """Обновить отображение полосы здоровья"""
        # Обновляем основной HUD
        ma in _health_id== "ma in _health_bar"
        if ma in _health_id in self.hud_elements:
            element== self.hud_elements[ma in _health_id]
    ...
    def _update_resource_d is play(self, bar_id: str, resource_bar: ResourceBar):
        """Обновить отображение полосы ресурса"""
            ...
            def _update_effects_d is play(self, entity_id: str):
    ...
    def toggle_hud(self, hud_type: HUDType):
        ...
        if current_time - self.last_update_time < self.auto_update_ in terval:
            return
    ...
    def get_v is ible_elements(self) -> L is t[HUDElement]:
        """Получить все видимые элементы"""
            return [e for e in self.hud_elements.values() if e.v is ible]:
            pass  # Добавлен pass в пустой блок
            def get_layout_elements(self, layout: HUDLayout) -> L is t[HUDElement]:
    ...
    def set_debug_mode(self, enabled: bool):
        """Включить / выключить режим отладки"""
            self.show_debug_ in fo== enabled

            # Показываем / скрываем отладочную информацию
            debug_element_id== "debug_ in fo"
            if debug_element_id in self.hud_elements:
            self.hud_elements[debug_element_id].v is ible== enabled
