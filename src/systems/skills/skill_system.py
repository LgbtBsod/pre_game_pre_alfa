...
class Skill:
    ...
    def _check_cooldown(self, user: Any) -> bool:
        """Проверить кулдаун"""
        if self.cost.cooldown <= 0:
            return True
    ...
    def _evaluate_condition(self, condition: str, user: Any, target: Optional[Any], context: Optional[Dict[str, Any]]) -> bool:
        """Оценить условие"""
        # TODO: Реализовать систему условий
        return True
...
class SkillSystem(BaseComponent):
    """
    Консолидированная система навыков
    ...
    def _register_skill_trees(self):
        ...
        combat_tree = {
            "basic_attack": ["improved_attack", "critical_strike"],
    ...
    def use_skill(self, entity_id: str, skill_id: str, target_id: Optional[str] = None, context: Optional[Dict[str, Any]] = None) -> bool:
        """Использовать навык"""
    ...
    def _apply_skill_cost(self, user: Any, skill: Skill):
        """Применить стоимость навыка"""
        if not user:
            return
    ...
    def _update_skill_cooldown(self, user: Any, skill: Skill):
        """Обновить кулдаун навыка"""
        if not user:
            return
        
        setattr(user, f'last_used_{skill.id}', time.time())
    
    def _update_combo_chain(self, entity_id: str, skill_id: str):
        """Обновить цепочку комбо"""
        ...
        if len(self.combo_chains[combo_key]) > 5:
            self.combo_chains[combo_key].pop(0)
    ...
    def use_skill_slot(self, entity_id: str, slot_index: int, target_id: Optional[str] = None, context: Optional[Dict[str, Any]] = None) -> bool:
    ...
    # Эффекты навыков
    def _damage_effect(self, effect: SkillEffect, user: Any, target: Optional[Any], context: Optional[Dict[str, Any]]):
        """Эффект урона"""
        # TODO: Интеграция с системой урона
        pass
    
    def _heal_effect(self, effect: SkillEffect, user: Any, target: Optional[Any], context: Optional[Dict[str, Any]]):
        """Эффект лечения"""
        # TODO: Интеграция с системой здоровья
        pass
    
    def _movement_effect(self, effect: SkillEffect, user: Any, target: Optional[Any], context: Optional[Dict[str, Any]]):
        """Эффект движения"""
        # TODO: Интеграция с системой движения
        pass
    
    # Публичные методы
    def get_skill_template(self, template_id: str) -> Optional[Skill]:
        """Получить шаблон навыка"""
        return self.skill_templates.get(template_id)
    
    def register_skill_template(self, template: Skill):
        """Зарегистрировать шаблон навыка"""
        self.skill_templates[template.id] = template
    
    def get_available_skills(self, entity_id: str) -> List[Skill]:
        """Получить доступные для изучения навыки"""
    ...
    def get_combo_chain(self, entity_id: str) -> List[str]:
        ...
        combo_key = f"{entity_id}_combo"
        return self.combo_chains.get(combo_key, [])
    
    def get_skill_tree(self, tree_name: str) -> Dict[str, List[str]]:
