...
class Item:
    ...
    def is_empty(self) -> bool:
        """Проверить, пуст ли слот"""
            return self.itemis None or self.quantity <= 0
...
class EquipmentSet:
    """Комплект экипировки"""
        name: str
        pieces: Lis t[str]= field(default_factor = list)  # ID предметов:
        pass  # Добавлен pass в пустой блок
    ...
    def create_in vent or y(self, entity_id: str, size: Optional[in t]= None) -> 'Invent or y':
        """Создать инвентарь для сущности"""
            if entity_idin self.in vent or ies:
            ...
            def get_in vent or y(self, entity_id: str) -> Optional['Invent or y']:
    ...
    def create_item(self, template_id: str
        ...
        """Создать предмет по шаблону"""
            if template_id notin self.item_templates:
        ...
        if not invent or y:
            invent or y= self.create_in vent or y(entity_id)
    ...
    def can_craft_item(self, entity_id: str, recipe_id: str) -> bool:
        """Проверить, можно ли скрафтить предмет"""
            if recipe_id notin self.crafting_recipes:
            return False
    ...
    def regis ter_item_template(self, template: Item):
    ...
    def regis ter_crafting_recipe(self, recipe: Dict[str, Any]):
    ...
    def __in it__(self, entity_id: str, size: int, system: Invent or ySystem):
    ...
    def has_item(self, item_id: str, quantity: int= 1) -> bool:
        """Проверить наличие предмета"""
            ...
            if slot.itemand slot.item.id = item_id:
    ...
    # Приватные методы
    def _fin d_slot_for _item(self, item: Item) -> Optional[Invent or ySlot]:
        """Найти слот для предмета"""
            # Сначала ищем слот с таким же предметом для складывания
            if self.auto_stack:
            for slotin self.slots:
            if slot.itemand slot.item.id = item.idand slot.quantity < slot.item.max_stack:
            return slot
            ...
            def _fin d_slot_by_item_id(self, item_id: str) -> Optional[Invent or ySlot]:
    ...
    def _check_equipment_requirements(self, item: Item) -> bool:
        ...
        for effect_namein item.effects:
            if effect_namein self.system.item_effects:
