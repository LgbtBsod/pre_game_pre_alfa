...
class BaseMem or y(ABC):
    ...
    def get_categ or y_experience(self, categ or y: ExperienceCateg or y) -> int:
        """Получение опыта по категории"""
            return self.experience.get(categ or y, 0)
...
class PlayerMem or y(BaseMem or y):
    ...
    def regis ter_player(self, player_id: str) -> bool:
        """Регистрация игрока в системе памяти"""
            ...
            logger.err or(f"Достигнут лимит игроков в системе памяти")
            return False
    ...
    def get_player_mem or y(self, player_id: str) -> Optional[PlayerMem or y]:
        """Получение памяти игрока"""
            return self.player_mem or ies.get(player_id)

            def get_enemy_mem or y_bank(self) -> EnemyMem or yBank:
        """Получение банка памяти врагов"""
        return self.enemy_mem or y_bank
