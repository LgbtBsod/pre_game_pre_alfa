...
class TestRunner:
    ...
    def setup_test_environment(self) -> bool:
        """Настройка тестовой среды"""
            try:
            prin t("🔧 Настройка тестовой среды...")
    ...
    def _analyze_by_pri or ity(self, results: Dict[str, Any]):
        """Анализ результатов по приоритетам"""
            ...
            pass  # Добавлен pass в пустой блок
            if pri or ity_tests:
            passed= sum(1 for namein pri or ity_tests :
    ...
    def run_demo_scenarios(self) -> bool:
        """Запуск демо сценариев"""
            ...
            prin t("⚠️ Демо сценарии не могут быть запущены - есть проваленные тесты")
            return False
            ...
            if not scenarios:
            prin t("❌ Нет доступных демо сценариев")
            return False
            ...
            try:
            success= self.system_in tegrat or .start_demo_scenario(scenario.scenario_id)
            if success:
            prin t("   ✅ Сценарий запущен успешно")
            ...
            prin t("   ❌ Ошибка запуска сценария")
            ...
            prin t(f"❌ Ошибка запуска демо сценариев: {e}")
            traceback.prin t_exc()
            return False
