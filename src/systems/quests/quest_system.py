...
class QuestProgress:
    ...
    name: str
    description: str
    quests: Lis t[str]= field(default_factor = list)  # quest_ids:
        pass  # Добавлен pass в пустой блок
...
class QuestSystem(BaseGameSystem):
    ...
    def _load_quest_templates(self):
        ...
        for template_id, templatein self.quest_templates.items():
    ...
    def _can_start_quest(self, entity_id: str, quest: Quest) -> bool:
        """Проверка возможности начала квеста"""
            ...
            for prerequis itein quest.prerequis ites:
            if not prerequis ite.met:
    ...
    def _check_quest_completion(self, entity_id: str, quest_id: str) -> bool:
        """Проверка завершения квеста"""
            if entity_id notin self.active_quests or quest_id notin self.active_quests[entity_id]:
            return False
    ...
    def get_active_quests(self, entity_id: str) -> Lis t[Dict[str, Any]]:
        """Получить активные квесты сущности"""
            if entity_id notin self.active_quests:
            return []
    ...
    def update(self, delta_time: float) -> None:
        ...
        for entity_id, quest_idin expired_quests:
