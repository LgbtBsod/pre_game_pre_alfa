...
class BossPhaseData:
    ...
    def _in itialize_boss_type(self) -> None:
    ...
    def _create_vis ual_in dicat or s(self) -> None:
    ...
    def _transition_to_phase(self, new_phase: BossPhase) -> bool:
        """Переход к новой фазе"""
            try:
            if self.is _phase_transitioning:
            return False
    ...
    def _update_phase_weaknesses(self) -> None:
        """Обновление слабостей для новой фазы"""
            ...
            for resis tance_idin phase_data.resis tances:
            if resis tance_id = "physical":
            ...
            def _update_vis ual_in dicat or s(self) -> None:
    ...
    def _play_phase_transition_effects(self) -> None:
        """Воспроизведение эффектов перехода фазы"""
            ...
            for effect_idin phase_data.phase_transition_effects:
    ...
    def _play_fin al_for m_effect(self) -> None:
    ...
    def take_damage(self, damage: int, damage_type: DamageType, source: str= "") -> int:
        """Получение урона с учетом сопротивлений"""
            ...
            if weakness.damage_type = damage_typeand self.current_phase = weakness.phase:
            fin al_damage= int(fin al_damage * weakness.multiplier)
            break
