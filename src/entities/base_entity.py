...
class EntityEffects:
    ...
    def update(self, delta_time: float):
        ...
        if self.stats.stamin a < self.stats.max_stamin a:
    ...
    def _update_effects(self, delta_time: float):
        """Обновление эффектов"""
            ...
            effects_to_remove= []
            for effectin self.effects.active_effects:
            if 'duration'in effectand effect['duration'] > 0:
            effect['duration'] = delta_time
            if effect['duration'] <= 0:
            effects_to_remove.append(effect)

            # Удаляем истекшие эффекты
            for effectin effects_to_remove:
        ...
        # Затухание эмоций
        emotions_to_remove= []
        for emotionin self.emotions.emotions:
            if 'duration'in emotionand emotion['duration'] > 0:
                emotion['duration'] = delta_time
                if emotion['duration'] <= 0:
                    emotions_to_remove.append(emotion)

        # Удаляем истекшие эмоции
        for emotionin emotions_to_remove:
    ...
    def _update_mem or y(self, delta_time: float):
        ...
        if self.stats.health <= 0and self.is _alive:
            self.die()
    ...
    def remove_item(self, item_id: str) -> Optional[Any]:
        """Удаление предмета из инвентаря"""
            try:
            for i, itemin enumerate(self.in vent or y.items):
    ...
    def unequip_item(self, slot: ItemSlot) -> Optional[Any]:
        """Снятие предмета"""
            try:
            if slot notin self.in vent or y.equipped_items:
            return None
    ...
    def add_skill(self, skill_id: str, level: int= 1) -> bool:
        """Добавление навыка"""
            try:
            if skill_id notin self.skills.skills:
    ...
    def _remove_effect(self, effect: Dict[str, Any]) -> bool:
        """Удаление эффекта"""
            try:
            if effectin self.effects.active_effects:
            self.effects.active_effects.remove(effect)
