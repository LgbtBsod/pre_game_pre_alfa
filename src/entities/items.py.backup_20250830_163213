...
class Item:
    ...
    def _check_requirements(self, user: Dict[str, Any]) -> bool:
        """Проверка требований для использования"""
            ...
            for skillin self.requirements.skills:
            if skill notin user_skills:
            return False

            # Проверка предметов
            user_items= user.get('items', [])
            for itemin self.requirements.items:
            if item notin user_items:
            return False
    ...
    def _apply_single_effect(self, effect: ItemEffect, target: Dict[str, Any]
        ...
        """Применение одного эффекта"""
            ...
            if stat_nameand stat_namein target:
    ...
    def _remove_equip_effects(self, user: Dict[str, Any]) -> bool:
        ...
        if self.item_type = ItemType.WEAPONand slot = ItemSlot.WEAPON:
            return True
    ...
    def _reduce_durability(self):
        """Уменьшение прочности предмета"""
            ...
            def get_in fo(self) -> Dict[str, Any]:
