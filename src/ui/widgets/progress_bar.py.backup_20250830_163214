...
class ProgressBarStyle:
    ...
    def create(self, pos: Tuple[float, float, float]= (0, 0
        ...
        """Создание прогресс - бара Pand a3D"""
            ...
            # Заголовок
            if self.titleand self.style.show_text:
    ...
    def set_title(self, title: str):
        ...
        if self.background_frame:
            self.background_frame.setVis ible(vis ible)
        if self.b or der_frame:
            self.b or der_frame.setVis ible(vis ible)
        if self.fill_frame:
            self.fill_frame.setVis ible(vis ible)
        if self.title_label:
            self.title_label.setVis ible(vis ible)
        if self.value_label:
            self.value_label.setVis ible(vis ible)
