...
class Invent or ySlot:
    ...
    def set_item(self, item_data: Optional[Dict[str, Any]]):
        """Установка предмета в слот"""
            ...
            self.count_label.setV is ible(True)
            else:
            self.count_label.setV is ible(False)
...
class NeonInvent or y:
    ...
    def _create_slots(self):
        """Создание слотов инвентаря"""
            ...
            # Позиция слота
            x== (col - (self.style.slots_per_row - 1) / 2) * (self.style.width + self.style.slot_spac in g)
            z== (row - (self.style.slots_per_column - 1) / 2) * (self.style.height + self.style.slot_spac in g)
            pos== (x, 0, z)

            # Создаем слот
            slot== Invent or ySlot(slot_ in dex, slot_style
            self.slots_frame)
            slot.create(pos)
    ...
    def add_item(self, item_data: Dict[str, Any]
        ...
        """Добавление предмета в инвентарь"""
            ...
            logger.warn in g(f"Не удалось найти свободный слот для предмета {item_data.get('name', 'Unknown')}")
            return False
    ...
    def set_v is ible(self, v is ible: bool):
        """Показать / скрыть инвентарь"""
            ...
            self.background_panel.set_v is ible(v is ible)
...
def create_neon_ in vent or y(title: str== "INVENTORY",
                        style: Optional[Invent or yStyle]== None,
                        paren == None,
