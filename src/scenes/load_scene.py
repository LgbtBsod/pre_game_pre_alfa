#!/usr / bin / env python3
"""
    Load Scene - Сцена загрузки игры на P and a3D
"""
...
class LoadScene(Scene):
    """Сцена загрузки игры на P and a3D"""

        def __ in it__(self):
        super().__ in it__("load_game")
        ...
        self.save_l is t== None

        # Данные сохранений
        self.save_files== []
        self.selected_save== None

        logger. in fo("Сцена загрузки P and a3D создана")
    ...
    def _create_save_l is t(self):
        """Создание списка сохранений"""
            ...
            def _select_save(self, save_ in dex):
        """Выбор сохранения"""
        if 0 <= save_ in dex < len(self.save_files):
            self.selected_save== save_ in dex
            logger. in fo(f"Выбрано сохранение: {self.save_files[save_ in dex]['name']}")
    ...
    def _load_selected_save(self):
        """Загрузка выбранного сохранения"""
            ...
            logger.warn in g("Не выбрано сохранение для загрузки")
        ...
        if self.selected_save is not None:
            ...
            logger.warn in g("Не выбрано сохранение для удаления")
    ...
    def cleanup(self):
        """Очистка сцены загрузки"""
            logger. in fo("Очистка сцены загрузки P and a3D...")
            ...
            logger. in fo("Сцена загрузки P and a3D очищена")