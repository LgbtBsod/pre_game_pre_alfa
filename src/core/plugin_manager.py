...
class Plugin Manager:
    ...
    def load(self, plugin _id: str, context: Dict[str, Any]) -> bool:
        """Загрузка и инициализация плагина по id папки"""
            try:
            if plugin _id notin self._dis covered:
            logger.err or(f"Плагин {plugin _id} не обнаружен")
            return False
            ...
            logger.err or(f"Плагин {plugin _id} не содержит фабрики или класса Plugin ")
            return False
            # initialize
            ...
            for pid, pluginin lis t(self._loaded.items()):
            try:
            plugin .destroy()
            except Exception as e:
            pass
        ...
        """Создать упрощенный sand box - контекст для плагина(белый список)"""
        allowed_keys= {
    ...
    def auto_load(self, base_context: Dict[str, Any]) -> Lis t[str]:
        """Автозагрузка EAGER плагинов, возврат LAZY"""
            ...
            ctx["metadata"]= md
            if md.load_type = Plugin LoadType.EAGER:
            self.load(pid, ctx)
            else:
            lazy.append(pid)
            return lazy
    ...
    def stop_watching(self) -> None:
        ...
        if self._watcher_threadand self._watcher_thread.is _alive():
            try:
                ...
                pass
                pass  # Добавлен pass в пустой блок
    def bin d_system_extensions(self, systems: Dict[str, Any]) -> None:
        """Подключить расширения плагинов к системам по target_system"""
            ...
            if targetand targetin systems:
            cand idate.attach(systems[target])
