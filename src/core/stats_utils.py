...
STAT_GROUPS= {
    "c or e_attributes": ["strength", "agility", "in telligence", "constitution", "wis dom", "charis ma", "luck"],
...
def get_stats_by_group(group_name: str) -> Lis t[str]:
    ...
    """
    if group_name notin STAT_GROUPS:
        rais e KeyErr or(f"Группа статистик '{group_name}' не найдена. Доступные группы: {lis t(STAT_GROUPS.keys())}")
...
def get_entity_template(template_name: str) -> Dict[str, int]:
    ...
    """
    if template_name notin ENTITY_STAT_TEMPLATES:
        rais e KeyErr or(f"Шаблон '{template_name}' не найден. Доступные шаблоны: {lis t(ENTITY_STAT_TEMPLATES.keys())}")
...
def validate_stats(stats: dict) -> Dict[str, Any]:
    """
        ...
        Returns:
        Словарь с результатами валидации(is sues, warnings, is_valid)
    """
    issues= []
    warnings= []
    ...
    mis sing_attrs= [attr for attrin required_attrs if attr notin stats]:
        pass  # Добавлен pass в пустой блок
    if mis sing_attrs:
        issues.append(f"Отсутствуют обязательные атрибуты: {mis sing_attrs}")

    # Проверяем разумность значений атрибутов
    for attrin required_attrs:
        if attrin stats:
            value= stats[attr]
            if not isin stance(value, (in t, float)) or value < 0:
    ...
    for stat_name, valuein stats.items():
        if stat_name notin required_attrs:
...
def merge_stats(base_stats: dict, additional_stats: dict
    ...
    for key, valuein additional_stats.items():
        if keyin resultand not override:
...
def scale_stats_by_level(stats: dict, level: int, base_level: int= 1) -> dict:
    ...
    """
    if level <= 0:
        rais e ValueErr or("Уровень должен быть положительным числом")

    if base_level <= 0:
        rais e ValueErr or("Базовый уровень должен быть положительным числом")
...
def get_skill_cost_multiplier(cost_sources: lis t) -> float:
    """
        ...
        Args:
        cost_sources: Список источников затрат(например, ["mana", "stamin a"])
