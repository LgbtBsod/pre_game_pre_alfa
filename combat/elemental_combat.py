...
class ElementalCombatSystem:
    ...
    def __init__(self):
        self.elemental_interactions: Dict[
            Tuple[ElementType, ElementType], ElementalInteraction
        ] = {}
    ...
    def _initialize_elemental_interactions(self):
        ...
        # Огонь + Лед = Пар
        self.elemental_interactions[(ElementType.FIRE, ElementType.ICE)] = (
            ElementalInteraction(
                source_element=ElementType.FIRE,
                target_element=ElementType.ICE,
                damage_multiplier=combat_settings.FIRE_ICE_MULTIPLIER,
                additional_effects=["steam", "blind"],
                chance=combat_settings.FIRE_ICE_CHANCE,
                description="Огонь и лед создают обжигающий пар",
            )
        )

        # Огонь + Вода = Пар
        self.elemental_interactions[(ElementType.FIRE, ElementType.WATER)] = (
            ElementalInteraction(
                source_element=ElementType.FIRE,
                target_element=ElementType.WATER,
                damage_multiplier=combat_settings.FIRE_WATER_MULTIPLIER,
                additional_effects=["steam", "slow"],
                chance=combat_settings.FIRE_WATER_CHANCE,
                description="Огонь испаряет воду, создавая пар",
            )
        )

        # Молния + Вода = Электропроводность
        self.elemental_interactions[(ElementType.LIGHTNING, ElementType.WATER)] = (
            ElementalInteraction(
        ...
        self.elemental_interactions[(ElementType.LIGHTNING, ElementType.ICE)] = (
            ElementalInteraction(
                source_element=ElementType.LIGHTNING,
                target_element=ElementType.ICE,
        ...
        # Земля + Огонь = Лава
        self.elemental_interactions[(ElementType.EARTH, ElementType.FIRE)] = (
            ElementalInteraction(
        ...
        self.elemental_interactions[(ElementType.WIND, ElementType.FIRE)] = (
            ElementalInteraction(
        ...
        # Свет + Тьма = Взрыв
        self.elemental_interactions[(ElementType.LIGHT, ElementType.DARK)] = (
            ElementalInteraction(
    ...
    def calculate_damage(
        ...
        target_element: ElementType,
        target_resistances: Dict[ElementType, float],
    ) -> CombatResult:
        ...
        interaction = self.elemental_interactions.get(
            (attacker_element, target_element)
        )
        effects_applied = []
        elemental_reactions = []
    ...
    def _check_elemental_combos(
        self, source_element: ElementType, target_element: ElementType
    ) -> Optional[str]:
    ...
    def _apply_single_effect(
        self, target, effect_data: Dict[str, Any], duration: float
    ):
            ...
            for nearby_target in nearby_targets:
                ...
                if self._can_chain_react(current_effect, nearby_target):
                    chain_reactions.append(
                        f"Цепная реакция {chain_level + 1}: {current_effect} -> {nearby_target}"
                    )
                    processed_targets.add(nearby_target)
                    targets_to_process.append(
                        (nearby_target, current_effect, chain_level + 1)
                    )
    ...
    def get_elemental_bonus(
        self, element: ElementType, target_resistances: Dict[ElementType, float]
    ) -> float:
