...
def is_point_in_rect(
    ...
    cx, cy = rect_center
    return (
        cx - width / 2 <= x <= cx + width / 2
        and cy - height / 2 <= y <= cy + height / 2
    )
...
def move_towards(
    ...
    direction = normalize_vector(
        (target_pos[0] - current_pos[0], target_pos[1] - current_pos[1])
    )
...
def calculate_experience(
    level: int, base_xp: int = 100, multiplier: float = 1.5
) -> int:
...
def calculate_level_from_xp(
    total_xp: int, base_xp: int = 100, multiplier: float = 1.5
) -> int:
