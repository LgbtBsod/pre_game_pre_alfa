...
class GameStateManager:
    ...
    def create_new_game(
        self, save_name: str, player_name: str, difficulty: str = "normal"
    ) -> str:
        """Создает новую игру"""
        try:
            import random
    ...
    def delete_save(self, game_id: str) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    "DELETE FROM save_statistics WHERE save_id = ?", (game_id,)
                )
                conn.commit()
    ...
    def update_player_state(self, player_data: Dict[str, Any]):
        ...
        try:
            ...
            if "position" in player_data:
                self.current_state.player_state.position = tuple(
            ...
            if "attributes" in player_data:
                self.current_state.player_state.attributes.update(
                    player_data["attributes"]
                )

            if "combat_stats" in player_data:
                self.current_state.player_state.combat_stats.update(
                    player_data["combat_stats"]
                )

            if "equipment" in player_data:
                self.current_state.player_state.equipment.update(
            ...
            if "achievements" in player_data:
                self.current_state.player_state.achievements = player_data[
                    "achievements"
                ]
