...
class Skill:
    ...
    def __init__(self, skill_id: str, name: str, skill_type: SkillType):
        ...
        # Получаем настройки умений
        from config.unified_settings import get_skill_settings

        skill_settings = get_skill_settings()

        # Базовые параметры
        self.level = 1
        self.max_level = skill_settings.MAX_SKILL_LEVEL
        self.cooldown = 0.0
    ...
    def can_learn(self, entity) -> bool:
        ...
        for skill_id in self.requirements.skills:
            if not hasattr(entity, "skill_system") or not entity.skill_system.has_skill(
                skill_id
            ):
                return False
...
class SkillSystem:
    ...
    def _init_basic_skills(self):
        ...
        basic_heal.effects = [SkillEffect("healing", 30.0, 0.0, "self")]
    ...
    def get_learned_skills(self) -> List[Skill]:
        ...
        return [skill for skill in self.skills.values() if skill.learned]

    def get_active_skills(self) -> List[Skill]:
        """Получить список активных умений"""
        return [
            skill for skill in self.skills.values() if skill.learned and skill.active
        ]
...
class SkillLibrary:
    ...
    def create_skill(skill_id: str, **kwargs) -> Skill:
        ...
        skill = Skill(
            skill_id,
            kwargs.get("name", skill_id),
            kwargs.get("skill_type", SkillType.ACTIVE),
        )
    ...
    def get_warrior_skills() -> List[Skill]:
        ...
        battle_spirit = SkillLibrary.create_skill(
            ...
            requirements=SkillRequirement(
                3, {AttributeType.STRENGTH: 15, AttributeType.VITALITY: 12}, []
            ),
    ...
    def get_mage_skills() -> List[Skill]:
        ...
        mage_shield = SkillLibrary.create_skill(
            ...
            mana_cost=20,
            cooldown=8.0,
            effects=[SkillEffect("magic_shield", 50.0, 8.0, "self")],
            requirements=SkillRequirement(
                3, {AttributeType.INTELLIGENCE: 15, AttributeType.FAITH: 10}, []
            ),
    ...
    def get_rogue_skills() -> List[Skill]:
        ...
        evasion = SkillLibrary.create_skill(
            ...
            effects=[SkillEffect("dodge_chance", 0.15, 0.0, "self")],
            requirements=SkillRequirement(
                3, {AttributeType.DEXTERITY: 15, AttributeType.LUCK: 10}, []
            ),
