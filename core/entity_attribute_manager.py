...
class EntityAttributeManager:
    ...
    def create_entity_attributes(self, entity_id: str, template_ids: Dict[str, float], level: int = 1) -> AttributeManager:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for template_id, current, maximum, growth, level in attributes_data:
                    attr = self.template_manager.create_attribute_from_template(template_id, level)
                    if attr:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for template_id, attr in attr_manager.attributes.items():
                    ...
                    """, (entity_id, template_id, attr.current, attr.maximum, attr.growth_rate, 1))
    ...
    def update_entity_attribute(self, entity_id: str, template_id: str, 
                              current_value: float = None, max_value: float = None):
        ...
        try:
            ...
            logger.error(f"Ошибка обновления атрибута {template_id} для сущности {entity_id}: {e}")
    ...
    def get_all_entity_attributes(self) -> List[Dict[str, Any]]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in results:
                    entity_id, template_id, current, maximum, growth, level, points, exp = row
    ...
    def export_entity_attributes_to_json(self, file_path: str = None):
        ...
        try:
            ...
            export_data = {
                "export_timestamp": str(Path(file_path).stat().st_mtime) if Path(file_path).exists() else "new",
