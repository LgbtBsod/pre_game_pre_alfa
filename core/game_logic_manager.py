...
class GameLogicManager:
    ...
    def _get_player_target(self) -> Optional[Tuple[float, float]]:
        ...
        if (
            self.game_state.chests
            and self.game_state.player.health < self.game_state.player.max_health * 0.7
        ):
        ...
        if alive_enemies:
            target = min(
                alive_enemies,
                key=lambda e: self._dist2(self.game_state.player.position, e.position),
            )
    ...
    def _update_entity_movement(self, delta_time: float) -> None:
        ...
        for enemy in self.game_state.enemies:
            if enemy.alive and self.game_state.player:
                ...
                self._move_entity_toward(
                    enemy, self.game_state.player.position, speed, delta_time
                )

        # Движение босса к игроку
        if (
            self.game_state.boss
            and self.game_state.boss.alive
            and self.game_state.player
        ):
            speed = self.game_state.boss.movement_speed
            self._move_entity_toward(
                self.game_state.boss, self.game_state.player.position, speed, delta_time
            )
    ...
    def _is_blocked_pixel(self, x: float, y: float) -> bool:
        ...
        tw, th = (
            self.game_state.tiled_map.tilewidth,
            self.game_state.tiled_map.tileheight,
        )
    ...
    def _process_chest_interactions(self) -> None:
        ...
        if (
            not self.game_state.chests
            or not hasattr(self.game_state, "tiled_map")
            or not self.game_state.tiled_map
        ):
            return
    ...
    def _get_nearest_chest_world_pos(
        self, from_pos: Tuple[float, float]
    ...
    def use_skill(self, skill_id: str) -> bool:
        ...
        success = self.game_state.skill_system.use_skill(
            skill_id, self.game_state.player
        )
        return success
