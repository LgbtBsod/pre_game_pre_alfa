...
class BaseEntity:
    ...
    def __init__(
        ...
        entity_id: str,
        position: tuple = (0, 0),
        attribute_templates: Dict[str, float] = None,
    ):
        ...
        if attribute_templates:
            ...
            try:
                from core.entity_attribute_manager import EntityAttributeManager

                entity_attribute_manager = EntityAttributeManager()
                # Убеждаемся, что entity_id - это строка
                entity_id_str = str(entity_id) if entity_id is not None else "default"
                loaded_attrs = entity_attribute_manager.load_entity_attributes(
                    entity_id_str
                )
                if loaded_attrs:
                ...
                logger.warning(
                    f"Не удалось загрузить атрибуты из БД: {e}, используем стандартные"
                )
                self.attribute_manager.initialize_default_attributes()
        ...
        self.learning_rate = self._get_ai_attribute(
            "learning_rate", UnifiedSettings.LEARNING_RATE_BASE
        )
        self.memory_decay_rate = self._get_ai_attribute(
            "memory_decay_rate", UnifiedSettings.MEMORY_DECAY_RATE_BASE
        )
        self.pattern_recognition_threshold = self._get_ai_attribute(
            "pattern_recognition_threshold",
            UnifiedSettings.PATTERN_RECOGNITION_THRESHOLD,
        )
    ...
    def take_damage(self, damage_report: dict):
        ...
        final_damage = self.combat_stats_manager.calculate_final_damage(
            damage, damage_type
        )
    ...
    def learn_from_damage(self, damage_report: dict):
        ...
        if damage_report.get("final_damage", 0) > 10:
            ...
            if attacker_weapon != "unknown":
                self.learn_weapon_effectiveness(
                    attacker_weapon, damage_type, damage_report.get("final_damage", 0)
                )
    ...
    def learn_weapon_effectiveness(
        self, weapon_type: str, damage_type: str, damage_amount: float
    ):
        ...
        if len(self.weapon_affinities[weapon_type][damage_type]) > 10:
            self.weapon_affinities[weapon_type][damage_type] = self.weapon_affinities[
                weapon_type
            ][damage_type][-10:]
    ...
    def select_best_weapon_for_target(self, target) -> Optional[dict]:
        ...
        if (
            target_type in self.weapon_affinities
            and weapon_type in self.weapon_affinities[target_type]
        ):
            # Используем обученные данные
            avg_damage = sum(self.weapon_affinities[target_type][weapon_type]) / len(
                self.weapon_affinities[target_type][weapon_type]
            )
    ...
    def learn_from_attack(self, damage_report: dict, target):
        ...
        if damage_dealt > 10:
            ...
            self.item_preferences[weapon_type] = max(
                0, self.item_preferences[weapon_type] - 1
            )
    ...
    def learn_item_usage(self, item, usage_type: str):
        ...
        self.ai_memory.record_event(
            "item_used",
            {"item_type": item_type, "usage_type": usage_type, "effectiveness": 1.0},
        )
    ...
    def set_ai_attribute(self, attribute_name: str, value: float):
        ...
        if hasattr(self, "attribute_manager") and self.attribute_manager:
            if self.attribute_manager.has_attribute(mapped_name):
                ...
                self.attribute_manager.set_attribute_base(mapped_name, value)
                self.attribute_manager.set_attribute_max(
                    mapped_name, value * 2
                )  # Максимум в 2 раза больше
    ...
    def save_attributes(self):
        ...
        try:
            ...
            entity_attribute_manager = EntityAttributeManager()
            entity_attribute_manager.save_entity_attributes(
                self.entity_id, self.attribute_manager
            )
        except Exception as e:
            import logging
