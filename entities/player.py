...
class Player(BaseEntity):
    ...
    def __init__(
        ...
        self.learning_rate = UnifiedSettings.LEARNING_RATE_PLAYER
    ...
    def analyze_death_cause(self):
        ...
        if recent_events:
            ...
            damage_types = [
                event.get("damage_type", "unknown") for event in recent_events
            ]
            attacker_types = [
                event.get("attacker_type", "unknown") for event in recent_events
            ]
    ...
    def learn_from_kill(self, enemy):
        ...
        self.ai_memory.record_event(
            "successful_kill",
            {
                ...
                "weapon_used": (
                    getattr(equipped_weapon, "item_type", "unknown")
                    if equipped_weapon
                    else "none"
                ),
                "level": self.level,
    ...
    def auto_heal_if_needed(self):
        ...
        if (
            "healing_potion" in self.item_preferences
            and self.item_preferences["healing_potion"] > 5
        ):
    ...
    def optimize_equipment_slot(self, slot_name: str):
        ...
        if best_item and (
            not current_item
            or best_score > self.calculate_item_score(current_item, slot_name)
        ):
